...
## Wall thickness for stretched walls — confirmed from tile_set.tscn's BoxMesh
...
const WATER_HOOKUP_PLACEMENT_Y: float = 3.2
...
func _get_device_extra(node: Node3D, tile_id: int) -> Dictionary:
	match tile_id:
		...
		TILE_STOVE:
			if "powered_on" in node:
				var extra: Dictionary = {"powered_on": node.get("powered_on")}
...
## WallDrawMode._confirm_wall() (live placement) and restore_placed_objects()
## (save/load), so the two can never produce different-looking walls.
## `pos` is the wall's MIDPOINT (matches how WallDrawMode already computes it).
func _spawn_stretched_wall(tile_id: int, pos: Vector3, angle_deg: float, length: float) -> StaticBody3D:
	var height: float = WALL_HEIGHT_M
	if tile_id == TILE_HALF_WALL:
		height = WALL_HEIGHT_M * 0.5
	elif tile_id == TILE_QUARTER_WALL:
		height = WALL_HEIGHT_M * 0.25
	...
	mi.mesh = box
	mi.position = Vector3(0.0, height * 0.5, 0.0)   ## Bottom-at-body-local-0 — see main plan's Part 1
	body.add_child(mi)
	...
	body.set_meta("tile_id", tile_id)
	body.set_meta("wall_length", length)   ## Read back by _get_device_extra() on save — see below
