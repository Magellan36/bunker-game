...
##
## Requires manual wiring at its ceiling mounting height. Wall-mounted devices
## alone use invisible local feeds from the grid beneath them.
##
...
# ─── PowerManager registration (manual connector) ────
func _register_deferred() -> void:
	var pm: PowerManager = get_tree().get_first_node_in_group("power_manager") as PowerManager
	if pm == null:
		return
	pm.begin_bulk()
	pm.register_consumer(str(get_instance_id()), TIER_WATTS.get(tier, 75.0),
		self, TIER_TYPE_TAG.get(tier, "grow_light_normal"), power_priority, true)
