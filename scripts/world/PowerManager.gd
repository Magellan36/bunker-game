...
## Canonical Y height for all wire-grid snap keys (must match WireDrawMode._WIRE_Y).
const WIRE_GRID_Y: float = 1.0

func register_wire_node(pos: Vector3, role: String, device_id: String = "") -> String:
	_pmdbg("[PM:REG] register_wire_node pos=%s role=%s device_id=%s" % [str(pos), role, device_id])
	...
	if _wire_nodes.has(key):
		var existing_role: String = _wire_nodes[key].get("role", "")
		...
		if existing_role in ["breaker", "generator", "battery"]:
			_pmdbg("[PM:REG] SKIP overwrite of %s node key=%s (new role=%s)" % [existing_role, key, role])
			return key
...
func get_wire_node_role(node_key: String) -> String:
	if _wire_nodes.has(node_key):
		return _wire_nodes[node_key].get("role", "")
	return ""
...
func get_wire_node_pos(node_key: String) -> Vector3:
	if _wire_nodes.has(node_key):
		return _wire_nodes[node_key].get("pos", Vector3.ZERO)
	return Vector3.ZERO
...
## that was on the old ring no longer has an exact match in _wire_nodes.  The
...
func get_wire_node_key_at_pos(pos: Vector3) -> String:
	...
	for wn: Dictionary in _wire_nodes.values():
		...
		if d < best_dist:
			...
			best_key  = wn["key"]
	if best_key != "":
		_pmdbg("[PM:SNAP] get_wire_node_key_at_pos: exact miss at %s — nearest fallback to %s (dist=%.3f)" % [
...
func _resolve_node_key(key: String) -> String:
	...
	for existing_key: String in _wire_nodes:
...
func register_wire_edge(
	...
	if _wire_edges.has(edge_id):
		...
		## _needs_resolve=false and the solver never runs (even if topology changed).
		if _bulk_depth > 0:
			_needs_resolve = true
		else:
			_solve_network()
	...
	var pos_a: Vector3 = _wire_nodes[node_a_id]["pos"]
	var pos_b: Vector3 = _wire_nodes[node_b_id]["pos"]
	...
	## an existing edge (i.e. the node exists in _wire_nodes but is not yet an
	...
	##
	## We must do this AFTER adding the new edge to _wire_edges so that
...
## Finds the FIRST scene WireSegment whose edge_id matches the given id.
## Used by _split_wire_edge_at() to reuse a visual that Pass C0 already
## spawned, instead of creating a duplicate overlapping tube.
## Returns null if no matching segment exists in the scene.
...
func _find_wire_segment_by_edge_id(target_eid: String) -> Node3D:
	if get_tree() == null or target_eid.is_empty():
		return null
...
func _spawn_or_reuse_wire_visual(edge_id: String, p_from: Vector3, p_to: Vector3) -> Node3D:
	if get_tree() == null or edge_id.is_empty():
		return null
	...
	var parent: Node = get_tree().get_first_node_in_group("main_world")
	if parent == null:
...
func _split_wire_edge_at(mid_key: String) -> void:
	...
	_pmdbg("[SPLIT] _split_wire_edge_at mid_key=%s  mid_pos=%s  total_edges=%d" % [
		mid_key, str(mid_pos), _wire_edges.size()])
	for edge: Dictionary in _wire_edges.values():
		...
		if a_key == mid_key and b_key == mid_key:
			_pmdbg("[SPLIT]   SKIP self-loop edge %s" % edge["id"])
		...
		if a_key == mid_key or b_key == mid_key:
			...
			_pmdbg("[SPLIT]   ENDPOINT edge %s  (a=%s b=%s) — falling through to split" % [edge["id"], a_key, b_key])
		if not _wire_nodes.has(a_key) or not _wire_nodes.has(b_key):
			_pmdbg("[SPLIT]   SKIP missing node edge %s" % edge["id"])
			continue
		...
		_pmdbg("[SPLIT]   check edge %s  a=%s b=%s  on_seg=%s  am=%.3f mb=%.3f" % [
			edge["id"], a_key, b_key, str(on_seg), am, mb])
		if on_seg:
			to_split.append(edge["id"])

	_pmdbg("[SPLIT] edges to split: %d → %s" % [to_split.size(), str(to_split)])

	for eid: String in to_split:
		if not _wire_edges.has(eid):
			_pmdbg("[SPLIT]   eid %s vanished before split loop, skipping" % eid)
			continue
		...
		var a_pos: Vector3 = _wire_nodes[a_key]["pos"] if _wire_nodes.has(a_key) else Vector3.ZERO
		var b_pos: Vector3 = _wire_nodes[b_key]["pos"] if _wire_nodes.has(b_key) else Vector3.ZERO
		...
		_pmdbg("[SPLIT]   SPLITTING eid=%s  a=%s b=%s  am_len=%.3f mb_len=%.3f  orig_node=%s" % [
		...
		if am_len < SNAP_GRID * 0.5 and mb_len < SNAP_GRID * 0.5:
			_pmdbg("[SPLIT]   SKIP: both halves degenerate, nothing to do")
		...
		if mid_key == a_key:
			_pmdbg("[SPLIT]   A→M: skipped — mid==a (breaker was endpoint A, M→B takes orig_node)")
		elif not _wire_edges.has(new_id_ab):
			...
			if am_len >= SNAP_GRID * 0.5 and orig_node != null and is_instance_valid(orig_node):
				...
				_pmdbg("[SPLIT]   A→M: reused orig_node edge_id=%s  endpoints %s→%s (inset)" % [
				...
				_pmdbg("[SPLIT]   A→M: degenerate, hid orig_node")
				...
				_wire_edges[new_id_ab]["node"] = am_node
				_pmdbg("[SPLIT]   A→M: spawned/reused visual (orig_node was null)  edge_id=%s  endpoints %s→%s" % [
					new_id_ab, str(a_pos), str(mid_am)])
			wire_edge_registered.emit(new_id_ab)
		else:
			_pmdbg("[SPLIT]   A→M: edge %s already exists, skipped" % new_id_ab)
		## Skip M→B when it is a self-loop (mid == b).
		## The A→M edge already covers the full original span in that case.
		if mid_key == b_key:
			_pmdbg("[SPLIT]   M→B: skipped — mid==b (breaker was endpoint B, A→M took orig_node)")
			continue
		...
		if not _wire_edges.has(new_id_mb):
			...
			if mb_len < SNAP_GRID * 0.5:
				_pmdbg("[SPLIT]   M→B: degenerate (%.3f < %.3f), no visual spawned" % [
					mb_len, SNAP_GRID * 0.5])
			elif get_tree() == null:
				_pmdbg("[SPLIT]   M→B: no scene tree, cannot spawn visual")
				...
				## the perimeter ring.
				var mid_role: String = (_wire_nodes.get(mid_key, {}) as Dictionary).get("role", "joint")
				var b_role:   String = (_wire_nodes.get(b_key,   {}) as Dictionary).get("role", "joint")
				if mid_role != "consumer" and b_role != "consumer":
					...
					if existing_mb != null:
						...
						mb_node = existing_mb
						_pmdbg("[SPLIT]   M→B: reused EXISTING WireSegment (no duplicate spawned)  edge_id=%s" % new_id_mb)
					else:
						var fallback_parent: Node = get_tree().get_first_node_in_group("main_world")
						if fallback_parent == null:
							fallback_parent = get_tree().get_current_scene()
						if fallback_parent != null:
							...
							if wire_script_fb != null:
								...
								mb_node = fb_seg
								_pmdbg("[SPLIT]   M→B: spawned fallback WireSegment (orig_node was null)  edge_id=%s  parent=%s" % [
									new_id_mb, fallback_parent.name])
							else:
								_pmdbg("[SPLIT]   M→B: ERROR — could not load WireSegment.gd for fallback")
						else:
							_pmdbg("[SPLIT]   M→B: orig_node null and no valid parent found, cannot spawn visual")
			else:
				...
				if am_len < SNAP_GRID * 0.5:
					...
					mb_node = orig_node
					_pmdbg("[SPLIT]   M→B: reused orig_node (A side was degenerate)  edge_id=%s" % new_id_mb)
				else:
					...
					if existing_mb2 != null and existing_mb2 != orig_node:
						...
						if wire_script != null:
							...
							mb_node = new_seg
							_pmdbg("[SPLIT]   M→B: spawned new WireSegment  edge_id=%s  endpoints %s→%s (inset)  node=%s" % [
								new_id_mb, str(mid_mb), str(b_pos), new_seg.name])
						else:
							_pmdbg("[SPLIT]   M→B: ERROR — could not load WireSegment.gd script")
			_wire_edges[new_id_mb] = {
				"id": new_id_mb, "node_a": mid_key, "node_b": b_key,
				"load_w": 0.0, "node": mb_node,
			}
			wire_edge_registered.emit(new_id_mb)
			_pmdbg("[SPLIT]   M→B: registered edge %s  node=%s" % [
				new_id_mb, "null" if mb_node == null else mb_node.name])
		else:
			_pmdbg("[SPLIT]   M→B: edge %s already exists, skipped" % new_id_mb)
	...
	if not far_a.is_empty() and not far_b.is_empty() and far_a != far_b:
		...
		for edge: Dictionary in _wire_edges.values():
			if edge["node_a"] == mid_key or edge["node_b"] == mid_key:
				to_remove.append(edge["id"])
		for eid: String in to_remove:
			_remove_wire_edge_internal(eid, true)   ## is_split=true → no scene free, no prune
...
func reconcile_wire_visuals() -> int:
	...
	for eid: String in _wire_edges:
		...
		if tube == null:
			...
			tube = _spawn_or_reuse_wire_visual(eid, pa, pb)
			if tube != null:
...
## is_split = true when called from _split_wire_edge_at: skip queue_free on the
## scene node (caller re-assigns it to the first replacement edge) and skip
## intermediate-joint pruning (the replacement edges need those nodes intact;
## also, pruning would break the zone-color registry by erasing the node_keys
## that get_wire_zones_with_colors() uses as stable zone anchors).
func _remove_wire_edge_internal(edge_id: String, is_split: bool = false) -> void:
	...
	if _wire_nodes.has(na) and _wire_nodes.has(nb):
		var pos_a: Vector3 = _wire_nodes[na].get("pos", Vector3.ZERO)
		var pos_b: Vector3 = _wire_nodes[nb].get("pos", Vector3.ZERO)
	...
	for e: Dictionary in _wire_edges.values():
	...
	for ikey: String in candidate_keys:
		...
		if not _wire_nodes.has(ikey):
			continue
		var wn: Dictionary = _wire_nodes[ikey]
		var role: String = wn.get("role", "joint")
		if role != "joint":
			continue   ## Device or breaker node — never auto-remove.
		_wire_nodes.erase(ikey)
		wire_node_unregistered.emit(ikey)
