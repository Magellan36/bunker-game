...
func _run() -> void:
	for path in TARGETS:
		...
		_check(resource != null and (not resource is Script or (resource as Script).can_instantiate()), "loads %s" % path)
	_test_cart()
...
func _test_storage_contract() -> void:
	...
	var storage := (load("res://scripts/ui/inventory/StorageUI.gd") as GDScript).new() as CanvasLayer
...
func _test_cart() -> void:
	...
	for _i in ShopCart.MAX_ITEMS:
		cart.change(1, 1)
	_check(not cart.change(1, 1) and cart.quantity(1) == ShopCart.MAX_ITEMS, "cart enforces bounded order size")
