...
## rebase tracks to MaleModel/, strip Hips position tracks, strip
## CharacterArmature root tracks. The descent is NOT baked into the clips —
## the game drives it with a smooth shared curve (the female clip's own
## armature descent is back-loaded: knees fold first, body drops late+rushed;
## see AdventurerModelController._lerp_sit_position).
