...
## Bakes the Lying Down.fbx clip into an AnimationLibrary .res, same pipeline
## as build_sit_animations.gd: rebase track paths to "MaleModel/", strip the
## armature-root position/rotation tracks (root motion is never consumed — the
## game drives position and the 90° side turn), strip any Hips position track.
##
## Run: godot --headless --path <project> --script res://tools/build_lying_down_anim.gd

const CLIP_PATH: String = "res://assets/models/player/lying_down.fbx"
const LIB_PATH: String = "res://assets/models/player/anims/lying_down_lib.res"

func _init() -> void:
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://assets/models/player/anims"))
	var ps: PackedScene = load(CLIP_PATH)
	...
	## root's position AND rotation, since the game owns both (the 90° side turn).
	...
	print("saved ", LIB_PATH, " err=", err, " anim len=", anim.length, " tracks=", anim.get_track_count())
	inst.free()
	quit(0)